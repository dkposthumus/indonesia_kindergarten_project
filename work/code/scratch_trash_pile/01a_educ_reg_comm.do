...
local indiv_control i.cohort birth_order kinder_spillover tot_num_vis_97 i.general_health_97
	...
	collect clear
keep if sample_comm == 1
* Let's begin with running a univariate regression 
collect has_hh_controls="NO" has_comm_controls="NO" has_indiv_controls="NO" has_mom_fe="NO", tag(model[(1)]): reg educ14 1.kinder_ever, vce(cluster mom_pidlink)
* Then let's run a regression with our full set of household controls
collect has_hh_controls="YES" has_comm_controls="YES" has_indiv_controls="NO" has_mom_fe="NO", tag(model[(2)]): reg educ14 urban_97##kinder_ever `hh_control' `comm_control', vce(cluster mom_pidlink)
* Then let's run a regression with our full set of individual controls (w/no household controls)
collect has_hh_controls="NO" has_comm_controls="NO" has_indiv_controls="YES" has_mom_fe="NO", tag(model[(3)]): reg educ14 urban_97##kinder_ever `indiv_control', vce(cluster mom_pidlink)
* Now let's run a regression with our full set of household and individual controls
collect has_hh_controls="YES" has_comm_controls="YES" has_indiv_controls="YES" has_mom_fe="NO", tag(model[(4)]): reg educ14 urban_97##kinder_ever `hh_control' `comm_control' `indiv_control', vce(cluster mom_pidlink)
...
* We have a key problem, immediately: our weights are no longer consistent throughout our panel variable. (?????? --> what to do?)
	...
	collect has_hh_controls="NO" has_comm_controls="NO" has_indiv_controls="NO" has_mom_fe="YES", tag(model[(5)]): xtreg educ14 1.kinder_ever, fe vce(robust)
		* Now, let's add in our individual controls.
	collect has_hh_controls="NO" has_comm_controls="NO" has_indiv_controls="YES" has_mom_fe="YES", tag(model[(6)]): xtreg educ14 1.kinder_ever `indiv_control', fe vce(robust)
* Now let's put all these results in a table together.
	collect label drop colname 
collect layout (colname[1.kinder_ever 1.urban_97 1.urban_97#1.kinder_ever]#result[_r_b _r_se] result[has_hh_controls has_comm_controls has_indiv_controls has_mom_fe r2_a N]) (model[(1) (2) (4) (6)])
	...
	collect preview
		collect export "$output/regression_results_comm.tex", tableonly replace
