...
//  program:    01_hh_education_variable_ifls4.do
//  task:		creating an education variable with the ifls4 data
...
local data_clean "/Users/danielposthumus/Thesis_Independent_Study/IFLS5/Work/Data_Clean/hh_7_data"
cd "`data_clean'"

* We are seeking to create our years_of_education variable from variables 		included in the b3a_dl1 datafile.
use ifls4_b3a_dl1, clear

* merge with household-identifying dataset, ifls5_b3a_cov
merge 1:1 hhid07 pid07 using ifls4_b3a_cov
* We want to remove those households missing their hhid from the general dataset, 		so we only keep those households that are matching according to both our criteria (hhid07 as well as pid07).
keep if _merge == 3
drop _merge
*******************************************************************************
...
local tag 01_hh_education_variables_ifls4.do
foreach v in educ educ_level evschl {
	notes `v': `tag'
}
compress
save ifls4_b3a_dl1_educ, replace
