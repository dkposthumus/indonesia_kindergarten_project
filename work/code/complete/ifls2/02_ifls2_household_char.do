/*
capture log close
log using 02_ifls2_household_char.do, replace text
*/

/*
  program:    	02_ifls2_household_char.do
...
reshape wide ks08x nfd_expend ks09ax nfd_consum, i(hhid97 version) j(ks3type) string
...
use b1_ks1, clear
tempfile food_month
rename (ks02 ks03) (fd_expend fd_consum)
reshape wide ks02x fd_expend ks03x fd_consum, i(hhid97 version) j(ks1type) string
	...
	local F "kangkung, cucumber, spinach, mustard greens, tomatoes, cabbge, katuk, green beans, string beans, etc."
...
/*
	...
	Stove indicator - unclear to me how to code. The possible values of k24 ("Kind of stove for cooking") are as follows: electricity, gas, ker[osene] stove, firewood, charcoal, other, do not cook, and dk. 
	
	How to code a binary variable about whether a household has a stove? My guess is that electric and gas stoves are considered true stoves, while the rest aren't. Unclear.
...
merge 1:1 hhid97 using `expenditure_97'
	drop if _merge != 3
	drop _merge

merge 1:1 hhid97 using `nonfood_year', nogen

merge 1:1 hhid97 using `household_indicators'
	drop if _merge != 3
	drop _merge
tempfile ifls2_hh_char
	save `ifls2_hh_char'
********************************************************************************
...
* Finish Up 
********************************************************************************
local tag 02_ifls2_household_char.do
...
compress
keep hhid97 ln_agg_expend_r_pc_97 electricity_97 television_97 fridge_97
* now let's label this dataset:
	label data "ifls2 household characteristics"

cd "$clean/ifls2_hh"
save ifls2_household_char, replace
