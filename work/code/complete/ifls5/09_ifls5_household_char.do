/*
capture log close
log using 09_ifls5_household_char, replace text
*/

/*
  program:    	09_ifls5_household_char.do
...
reshape wide ks08x nfd_expend ks09ax nfd_consum, i(hhid14_9 version) j(ks3type) string
...
reshape wide ks02x fd_expend ks03x fd_consum, i(hhid14_9 version) j(ks1type) string
...
* Merge datsets ***************************************************************************************
use `food_month'
merge 1:1 hhid14_9 using `nonfood_year', nogen
merge 1:1 hhid14_9 using `household_indicators'
drop if _merge != 3
drop _merge
tempfile ifls5_hh_char
save `ifls5_hh_char'
***************************************************************************************
...
*/
use `ifls5_hh_char', clear
* Since there's no master data for consumption/expenditure here in ifls5, let's sum the two categories of expenditure ourselves and use that as a proxy:
ds nfd_expend*
egen total_nfd_expend = rowtotal(`r(varlist)')
	label var total_nfd_expend "total non-food monthly expenditure"
ds fd_expend* 
egen total_fd_expend = rowtotal(`r(varlist)') 
	label var total_fd_expend "total food monthly expenditure"
gen agg_expend = total_nfd_expend + total_fd_expend 
	label var agg_expend "aggregate monthly expenditure"
save `ifls5_hh_char', replace
* Now we run into a problem: there's no household size variable. Let's quickly bring in the household roster and use that figure as the hhsize variable:
cd "$raw/ifls5_hh" 
	use bk_ar1, clear
bysort hhid14_9: egen hhsize = max(pid14)
	keep hhid14_9 hhsize
* We can just drop the duplicates of hhid14_9, since hhsize is constant within hhid14_9:
		duplicates drop hhid14_9, force
merge 1:1 hhid14_9 using `ifls5_hh_char'
	gen ln_agg_expend_pc = ln(agg_expend)/hhsize
		keep if _merge == 3
***************************************************************************************
* Finish Up 
***************************************************************************************
local tag 09_ifls5_household_char.do
...
keep hhid14_9 ln_agg_expend_pc_14 electricity_14 television_14 fridge_14
label data "ifls5 household characteristics"
compress

cd "$clean/ifls5_hh"
save ifls5_household_char, replace
