/*
capture log close
log using 92_reg_switch, replace text
*/

/*
  program:    	92_reg_switch
...
foreach v in educ14 elem_completion junior_completion senior_completion in_schl1 in_schl2 in_schl3 in_schl4 in_schl5 in_schl6 in_schl7 in_schl8 in_schl9 in_schl10 in_schl11 in_schl12 in_schl13 in_schl14 stayon6 stayon9 stayon12 {
		 collect clear 
		 collect, tag(model[`v']): qui reg `v' kinder_ever `control', vce(robust)
		 collect, tag(model[`v'_sw]): qui reg `v' kinder_ever `control' if switch == 1, vce(robust)
...
* now cognitive test scores: 
		...
		foreach v in 00 07 14 {
		 collect clear 
		 collect, tag(model[`v']): qui reg ln_total_ek`v' kinder_ever `control', vce(robust)
		 collect, tag(model[`v'_sw]): qui reg ln_total_ek`v' kinder_ever `control' if switch == 1, vce(robust)
		collect layout (colname[kinder_ever]#result[_r_b _r_se]) (model)
}
