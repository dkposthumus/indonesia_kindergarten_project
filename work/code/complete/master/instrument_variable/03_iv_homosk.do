/*
capture log close
log using 03_iv_homosk, replace text
*/

/*
  program:    	03_iv_homosk
	...
	preserve 
		qui ivregress gmm `v' (kinder_ever = `instrument') `hh_control' `indiv_control' `comm_control', wmatrix(robust) 
			...
			predict yhat, xb 
				...
				sum uhat if kinder_ever == 0 
				sum uhat if kinder_ever == 1
				scatter uhat kinder_ever, nodraw saving(`v'.gph, replace) title("`v'") yline(0) msize(tiny)
	...
	keep if ln_total_ek00 != . & ln_total_ek07 != . & ln_total_ek14 != . 
		qui ivregress gmm ln_total_ek`v' (kinder_ever = `instrument') `hh_control' `indiv_control' `comm_control', wmatrix(robust) 
			...
			predict yhat, xb 
				...
				sum uhat if kinder_ever == 0 
				sum uhat if kinder_ever == 1
				scatter uhat kinder_ever, nodraw saving(ln`v'.gph, replace) title("ln_total_ek`v'") yline(0) msize(tiny)
